...
test.describe('Phase 1 Foundational Architecture & Layout', () => {
  test('renders page without console errors and contains accessible landmarks', async ({ page }) => {
    const consoleErrors = [];
    page.on('console', msg => {
    ...
    await expect(sheet).toHaveAttribute('aria-label', 'Last Will and Testament Document');
  });

  test('skip link becomes visible and accessible on focus', async ({ page }) => {
    await page.goto('/');
  ...
  test('form inputs are properly associated with labels and fieldsets', async ({ page }) => {
    await page.goto('/');
  ...
  test('document structure contains 10 articles, execution, and attestation blocks', async ({ page }) => {
    await page.goto('/');
  ...
  test('zoom toolbar triggers data-zoom attribute on sheet container', async ({ page }) => {
    await page.goto('/');
  ...
  test('print media emulation hides application UI and preserves document sheet', async ({ page }) => {
    await page.goto('/');
  ...
  test('ensures sidebar and document viewport have independent scroll containers on desktop', async ({ page, isMobile }) => {
    test.skip(isMobile, 'Desktop specific layout verification');
    await page.goto('/');

    const sidebarScrollable = await page.locator('aside.app-sidebar').evaluate(el => {
      const style = window.getComputedStyle(el);
      return style.overflowY === 'auto' || style.overflowY === 'scroll';
    });
    expect(sidebarScrollable).toBe(true);

    const viewportScrollable = await page.locator('main.document-viewport').evaluate(el => {
      const style = window.getComputedStyle(el);
      return style.overflowY === 'auto' || style.overflowY === 'scroll';
    });
